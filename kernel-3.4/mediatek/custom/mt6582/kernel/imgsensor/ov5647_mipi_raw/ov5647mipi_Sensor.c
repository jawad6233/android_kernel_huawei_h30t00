...
UINT32 OV5647MIPISetMaxFrameRate(UINT16 u2FrameRate)
{
	...
	spin_unlock_irqrestore(&ov5647mipi_drv_lock,flags);
		if((ov5647CurrentScenarioId == MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG)||(ov5647CurrentScenarioId == MSDK_SCENARIO_ID_CAMERA_ZSD)){
		  dummy_line = FrameHeight - OV5647MIPI_FULL_PERIOD_LINE_NUMS;
		}
		else if(ov5647CurrentScenarioId==MSDK_SCENARIO_ID_CAMERA_PREVIEW){
			dummy_line = FrameHeight - OV5647MIPI_PV_PERIOD_LINE_NUMS;
		}
		else if(ov5647CurrentScenarioId==MSDK_SCENARIO_ID_VIDEO_PREVIEW) {
