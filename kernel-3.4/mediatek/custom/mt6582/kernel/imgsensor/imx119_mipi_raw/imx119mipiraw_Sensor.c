...
#define IMX119_TEST_PATTERN_CHECKSUM (0x3e3a470f)
kal_uint8  test_pattern_flag=0;


static struct IMX119MIPI_sensor_STRUCT IMX119MIPI_sensor={IMX119MIPI_WRITE_ID,IMX119MIPI_READ_ID,KAL_TRUE,KAL_FALSE,KAL_TRUE,KAL_FALSE,KAL_FALSE,
...
void IMX119MIPI_write_shutter(kal_uint16 shutter)
{
    ...
    if (IMX119MIPI_sensor.pv_mode == KAL_TRUE||IMX119MIPI_sensor.video_mode== KAL_TRUE) 
	 {
	...
	if(IMX119MIPI_Auto_Flicker_mode== KAL_TRUE)
	{
		...
		if((realtime_fp >= 298)&&(realtime_fp <= 302))
		{
			...
			frame_length = IMX119MIPI_sensor.pv_pclk *10 / (line_length * realtime_fp);
...
UINT32 IMX119MIPIGetSensorID(UINT32 *sensorID) 
{
	...
	SENSORDB("[IMX119MIPI]exit IMX119MIPIGetSensorID function\n");
    return ERROR_NONE;
}
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(CurrentScenarioId==MSDK_SCENARIO_ID_VIDEO_PREVIEW)    
		IMX119MIPI_sensor.video_mode=KAL_TRUE;
	else
		IMX119MIPI_sensor.video_mode=KAL_FALSE;
...
UINT32 IMX119MIPISetTestPatternMode(kal_bool bEnable)
{
    ...
    if(bEnable) {   // enable color bar
        ...
        IMX119MIPI_write_cmos_sensor(0x301a, 0x01);  // color bar test pattern
    ...
    } else 
    {
        ...
        //IMX119MIPI_write_cmos_sensor(0x30D8, 0x10);  // color bar test pattern
        IMX119MIPI_write_cmos_sensor(0x301a, 0x00);  // disable color bar test pattern
    }
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	{
		test_pattern_flag=false;
		IMX119MIPISetTestPatternMode(true);
	}
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
		...
		spin_unlock(&imx119_drv_lock);
		SENSORDB("[IMX119MIPI]enter IMX119MIPIControl function ScenarioId=%s\n",ScenarioId);
    switch (ScenarioId)
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
...
UINT32 IMX119MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	SENSORDB("[IMX119MIPI]enter IMX119MIPISetVideoMode function u2FrameRate=%s\n",u2FrameRate);
	    if(u2FrameRate ==0)
			return TRUE;
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
		...
		case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE:
			*pFeatureReturnPara32 = IMX119_TEST_PATTERN_CHECKSUM;
			*pFeatureParaLen=4;
        default:
            break;
    }
