...
/* Add the LM3642 fl driver :i2c device & driver varabiles */
...
#define CURRENT_BRIGHT_LOW_DUTY    0
#define CURRENT_BRIGHT_MEDIUM_DUTY 1 //15  cust down driving current for safe
#define CURRENT_BRIGHT_HIGH_DUTY  2 // 12  //20 cust down driving current for safe
        ...
        const char *buf, size_t count)
{
    ...
    {
        if((HW_G750_VER == boardType))
        { //For G750
            if (g_pTorchFunc) //set touch 
            {
                ...
                /*Turn on the flash.*/
                //add flashlight preon port for G750-T00
                g_pTorchFunc->flashlight_ioctl(FLASH_IOC_PRE_ON, 0);
                mdelay(13);
...
/* Add the LM3642 fl driver. Register i2c num 2 bus info*/
