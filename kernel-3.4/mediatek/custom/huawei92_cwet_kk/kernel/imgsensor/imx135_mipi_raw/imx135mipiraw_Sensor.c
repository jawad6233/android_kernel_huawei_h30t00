...
#define IMX135_LSC_OTP_Enable 1
#if IMX135_LSC_OTP_Enable 
#define LSC_OTP_DATA_SIZE 512
#define LSC_OTP_INVAILD_NG 0x00
#define LSC_OTP_VAILD_OK   0xFF
static kal_uint8 lsc_otp_data[LSC_OTP_DATA_SIZE]; 
static int lsc_otp_vaild_flag = LSC_OTP_INVAILD_NG;
#endif
...
kal_uint8 IMX135_awb_otp_vaild_check(void)
{
    kal_uint16 r_gr_h = 0;
    kal_uint16 r_gr = 0;
    kal_uint16 b_gr_h = 0;    
    kal_uint16 b_gr = 0;
    kal_uint16 gb_gr_h = 0;
    kal_uint16 gb_gr = 0;
        
    r_gr_h = IMX135OTPReadSensor(0x3B09) & 0x00ff;
    r_gr = IMX135OTPReadSensor(0x3B0A) & 0x00ff; 
    r_gr = r_gr | (r_gr_h << 8);
    SENSORDB("awb opt: r_gr=0x%x; \n",r_gr);
    
    b_gr_h = IMX135OTPReadSensor(0x3B0B) & 0x00ff;
    b_gr = IMX135OTPReadSensor(0x3B0C) & 0x00ff;
    b_gr = b_gr | (b_gr_h << 8);
    SENSORDB("awb opt: b_gr=0x%x; \n",b_gr);

    gb_gr_h = IMX135OTPReadSensor(0x3B0D) & 0x00ff;
    gb_gr = IMX135OTPReadSensor(0x3B0E) & 0x00ff;
    gb_gr = gb_gr | (gb_gr_h << 8);//high bit left shit 8
    SENSORDB("awb opt: gb_gr=0x%x; \n",gb_gr);

    if(r_gr && b_gr && gb_gr)
    {
        SENSORDB("awb opt params vaild!\n");  
        return 1;
    }    
    else
    {
        SENSORDB("awb opt params invaild!\n");  
        return 0;
    }    
}
...
kal_uint8 check_IMX135_otp_valid_LSC_Page(void)
{
	...
	kal_uint8 Pagemin = 9;
	kal_uint8 page = 0;
	kal_uint16 lsc_flag = 0x00ff;
    kal_uint8 awb_flag = 0x0000;
	...
	for(page = Pagemax ; page>=Pagemin;page--)
	{
	    if (IMX135_goto_page(page))
		{
			...
			lsc_flag = IMX135OTPReadSensor(0x3b0f);
            awb_flag = IMX135_awb_otp_vaild_check();
            SENSORDB("lscopt:: lsc_flag=%x; awb_flag=%x; \n",lsc_flag,awb_flag);
			if ((LSC_OTP_VAILD_OK == lsc_flag) && awb_flag )
			{
                lsc_otp_vaild_flag = LSC_OTP_VAILD_OK;
...
kal_bool IMX135_Read_LSC_Otp(kal_uint8 pagestart,kal_uint8 pageend)
{
	...
	for(page = pagestart; page<=pageend; page++)
	{
		...
		{
		    lsc_otp_data[i+number*64]=IMX135OTPReadSensor(LSCOTPaddress+i);
		    //SENSORDB("lsc_otp_data[%d] = %d\n",(i+number*64),lsc_otp_data[i+number*64]);
		}
		number ++;	
	}
	return KAL_TRUE;
...
void IMX135_Update_Lsc(kal_uint8 * dataBuf)
{
	...
	for(i = 0; i <LSC_OTP_DATA_SIZE-8; i++)
	{
...
//quality
   ...
   // ARNR Parameter setting
	...
	#if IMX135_LSC_OTP_Enable
    SENSORDB("lscopt: lsc_otp_vaild_flag=0x%x +\n",lsc_otp_vaild_flag);
    if(LSC_OTP_VAILD_OK != lsc_otp_vaild_flag)
    {    
        SENSORDB("lscopt: lsc_otp_vaild_flag=0x%x +\n",lsc_otp_vaild_flag);        
        
		if(IMX135_Read_LSC_Otp(1,8))
		{
			IMX135_Update_Lsc(lsc_otp_data);
			SENSORDB("LSC OTP update done \n");
		}
		else
		{
            IMX135OTPWriteSensor(0x0700, 0x00);// close lsc otp
    ...
    {
        IMX135_Update_Lsc(lsc_otp_data);
        SENSORDB("lscopt: Got lsc_otp_data paras, so directly use it.\n");
        SENSORDB("lscopt: LSC OTP update done \n");
    }
	#endif
...
UINT32 IMX135MIPIGetSensorID(UINT32 *sensorID) 
{
    ...
    //Avoid unnecessary camera search process
