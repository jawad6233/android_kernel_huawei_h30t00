...
kal_uint16 OV5648MIPI_read_otp_wb(kal_uint16 index, struct OV5648MIPI_otp_struct *otp_ptr)
{
    ...
    if(index == 2)  //Group 2
    {
		...
		msleep(50); // delay 50ms
        (*otp_ptr).rg_ratio = (OV5648MIPI_read_cmos_sensor(0x3d05) << 8) + OV5648MIPI_read_cmos_sensor(0x3d06);	
		(*otp_ptr).bg_ratio = (OV5648MIPI_read_cmos_sensor(0x3d07) << 8) + OV5648MIPI_read_cmos_sensor(0x3d08);
        (*otp_ptr).gb_gr_ratio = (OV5648MIPI_read_cmos_sensor(0x3d09) << 8) + OV5648MIPI_read_cmos_sensor(0x3d0a);
    }
    else if(index == 1) //Group 1
    {
    	//read otp bank 0
