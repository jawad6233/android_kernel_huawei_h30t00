...
/* Exclude invalid segments */
static void valid_pasr_range(unsigned long *start_pfn, unsigned long *end_pfn)
{
	unsigned long spfn = *start_pfn;
	unsigned long epfn = *end_pfn;
	unsigned long last_valid = spfn;
	unsigned long sseg, eseg;
	int valid;

	for ( ; spfn < epfn; spfn += pasrbank_pfns) {
		/* Check validness of current segment */
		sseg = spfn;
		eseg = sseg + pasrbank_pfns;
		valid = 0;
		for ( ; sseg < eseg; sseg++) {
			if (pfn_valid(sseg)) {
				valid++;
		...
		/* DFS case */
		if (valid >= pasrbank_pfns - 1) {
			last_valid = eseg;
		} else {
			/* PASR range should be continuous */
			break;
		}
	}
...
void __meminit init_mtkpasr_range(struct zone *zone)
{
	...
	/* Exclusive check - to decide actual end_pfn */
	valid_pasr_range(&start_pfn, &end_pfn);

	/* Output MTKPASR Start/End PFN */
