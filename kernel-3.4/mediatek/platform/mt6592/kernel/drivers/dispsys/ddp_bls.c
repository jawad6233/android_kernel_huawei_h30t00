...
#define MIN_VALUE_DUTY_AAL    32
#define MAX_VALUE_DUTY_AAL   983
//avoid 12.5% duty brightness value for TI backlight driver chip bug
static unsigned int min_value_duty_one_eight_aal= 125;  //1024*12.3%
static unsigned int max_value_duty_one_eight_aal = 129; //1024*12.5%
//avoid 25% duty brightness value for TI backlight driver chip bug
static unsigned int min_value_duty_one_four_aal= 253;   //1024*24.8%
static unsigned int max_value_duty_one_four_aal = 257;  //1024*25.2%
//avoid 50% duty brightness value for TI backlight driver chip bug
static unsigned int min_value_duty_one_two_aal = 509;   //1024*49.8%
static unsigned int max_value_duty_one_two_aal = 513;   //1024*50.2%
...
void disp_onConfig_bls(DISP_AAL_PARAM *param)
{
    ...
    if((0< level ) && (MIN_VALUE_DUTY_AAL>= level) )
    {
        // MIN_VALUE_DUTY_AAL/1023 = 3.1%  > (500ns*50.8KHz)
        level = MIN_VALUE_DUTY_AAL;
    }
    else if(MAX_VALUE_DUTY_AAL <= level)
    {
        // MAX_VALUE_DUTY_AAL/1023 = 96.89%  < 1-(500ns*50.8KHz)
        level = MAX_VALUE_DUTY_AAL;
    }
    else
    {
        BLS_DBG("[LED]Backlight Range is 32-983\n");
    }
    if((level >= min_value_duty_one_eight_aal) && (level <= max_value_duty_one_eight_aal)) //12.5% duty shanshuo
    {
        //avoid 12.5% duty brightness value for TI backlight driver chip bug
        level = min_value_duty_one_eight_aal-1;
    }
    else if((level >= min_value_duty_one_four_aal) && (level <= max_value_duty_one_four_aal))
    {
        //avoid 25% duty brightness value for TI backlight driver chip bug
        level = min_value_duty_one_four_aal-1;
    }
    else if((level >= min_value_duty_one_two_aal) && (level <= max_value_duty_one_two_aal))
    {
        //avoid 50% duty brightness value for TI backlight driver chip bug
        level = min_value_duty_one_two_aal-1;
    }
