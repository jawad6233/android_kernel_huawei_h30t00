...
DEFINE_MUTEX(Backlight_update);
BOOL LCM_CABC_update = FALSE;
...
int mtkfb_set_backlight_level(unsigned int level)
{
	...
	//Press the power button,can not wake up the phone probability
	mutex_lock(&Backlight_update);
	LCM_CABC_update = TRUE;
	mutex_unlock(&Backlight_update);
...
#endif
    MSG_FUNC_LEAVE();
    DISP_SetDeviceId();/*set LCD_ID to app_info*/
    return 0;
